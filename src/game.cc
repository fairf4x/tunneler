...
Color mud_color = {.r=168, .g=92, .b=15, .a=255};
Color rock_color = {.r=77, .g=77, .b=77, .a=255};
Color path_color = {.r=165, .g=140, .b=123, .a=255};
Color stru_color = {.r=0, .g=0, .b=120, .a=255};
...
game::game(bool split,int map_input)
{
	...
	if(SDL_Init(SDL_INIT_FLAGS)!=0)
	{
		...
		fprintf(stderr,"Fatal error: SDL_Init error: %s\n",SDL_GetError());
		SDL_Quit();
	}
	
	window = SDL_CreateWindow("Tunneler",
	...
	if(screen==NULL)
	{
		...
		fprintf(stderr,"Fatal error: SDL_CreateWindow error: %s\n",SDL_GetError());
		SDL_Quit();
	}
